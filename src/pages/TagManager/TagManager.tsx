...
export const TagManager = () => {
  ...
  const defaultFooter = () => <Button style={{width: '100%'}} icon={<PlusOutlined/>} type='dashed'>Tạo thẻ mới</Button>;
  ...
  return (
    <div className='tag-manager-page'>
      ...
      <Row gutter={[20, 20]}>
        ...
        <Col span={8} ><AdminInfoBlock description='Thẻ nổi bật' componentInfo={<div className='popular-tags'>{popularTag.map((tag) => <Tag style={{ borderRadius: '2em', fontSize: '0.9rem' }} color={tag.colorcode}>{`${tag.content}: ${tag.postusetag}`}</Tag>)}</div>} icon={<ProjectOutlined />} /></Col>
      </Row>
      ...
      <div className="tag-table-container">
        ...
        <Table className="tag-table"
          ...
          >
          <Table.Column title="Tên thẻ" render={(tag: ITag) => <span>{tag.content}</span>} width="10%" />
          <Table.Column title="Mô tả" render={(tag: ITag) => <span>{tag.description}</span>} width="60%" />
          <Table.Column title="Icon" render={(tag: ITag) => <i style={{fontSize: '2em'}} className={tag.icon_class}></i>} width="7%" />
          <Table.Column title="Mã màu" render={(tag: ITag) => <Tag color={tag.colorcode} style={{padding: 3, borderRadius: '2em'}}>{tag.colorcode}</Tag>} width="13%" />
